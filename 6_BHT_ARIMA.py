...
for i in tqdm(range(n_nodes)):
    ...
    def simulation(j):
        ...
        ur = BHTARIMA(p=x_train.shape[1], d=0, q=0, taus=[x_train.shape[0], tau],
                      Rs=[tau, tau], K=200, tol=0.001, verbose=0, Us_mode=4)
        ur.fit(x_ur_train, y_train[:, j])
        r = BHTARIMA(p=x_train.shape[1], d=0, q=0, taus=[x_train.shape[0], tau],
                     Rs=[tau, tau], K=200, tol=0.001, verbose=0, Us_mode=4)
        r.fit(x_r_train, y_train[:, j])
        y_ur_test_hat = ur.transform(x_ur_test)
        y_r_test_hat = r.transform(x_r_test)
    ...
    parallel_results = joblib.Parallel(n_jobs=5)(  # depends on your machine's memory size
...
# %% Export results
with open('raw/lorenz96_conditional_BHT_ARIMA_wilcoxon_stats.pkl', 'wb') as f:
...
heatmap.get_figure().savefig('results/lorenz96_conditional_BHT_ARIMA_wilcoxon.svg')
plt.close(fig)
